...
def test_lmit_wiki_parser_exposes_sync_stop_and_resume_commands():
    ...
    stop_args = parser.parse_args(["sync-stop", "--config", "config/wiki-only.example.toml"])
    resume_args = parser.parse_args(["sync-resume", "--config", "config/wiki-only.example.toml"])

    assert stop_args.config.name == "wiki-only.example.toml"
    assert resume_args.config.name == "wiki-only.example.toml"
